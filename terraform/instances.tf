resource "yandex_compute_instance" "app-server-1" {
  ...
  network_interface {
    ...
    nat                 = true
    security_group_ids  = [yandex_vpc_security_group.terra-appservers-sg.id]
  }
  ...
  scheduling_policy {
    ...
    depends_on = [yandex_mdb_postgresql_cluster.terradb, yandex_mdb_postgresql_database.db_name]
